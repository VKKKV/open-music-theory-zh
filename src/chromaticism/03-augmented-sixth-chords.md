...
增六和弦（Augmented Sixth Chords）Brian Jarvis
核心要点

- 增六和弦是一组半音下属和弦，包含 le 和 fi 之间的增六度 $(\downarrow\hat6-\uparrow\hat4)$。
- 有意大利增六和弦（It+6）、法国增六和弦（Fr+6）和德国增六和弦（Ger+6）三种。
- 增六和弦没有根音，它们解决到原位属和弦。
...
增六和弦是一类半音下属和声，其名称来源于包含一个非常特定的音程：le 和 fi 之间的增六度 $(\downarrow\hat6–\uparrow\hat4)$。每个增六和弦（意大利、法国和德国）除了这个音程外，还包含一个或两个其他音级，如下方图表所总结并如例 1 所示。增六和弦可以出现在大调和小调中，但在小调中更为常见。
...
你现在已经熟悉了为罗马数字分析寻找和弦根音的过程，但增六和弦通常不按根音分类。相反，它们被识别为包含 le 和 fi 之间增六度 $(\downarrow\hat6–\uparrow\hat4)$ 的和弦，并根据和弦中的其他音符进一步分类，如上所示。"意大利"、"法国"和"德国"这些名称比历史更具色彩性，但每个和弦都基于这个特征音程。
...
由于增六和弦不像你习惯的那样基于根音，你需要另一种策略来找到它们。试图通过三度叠置来确定性质会变得混乱，因为它会包含一个减三度。最简单的方法是记住低音运动 le–sol $(\downarrow\hat6-\\hat5)$ 可以支持这个进行，如果和弦出现在这些音级之上，并且包含 le $(\downarrow\hat6)$ 的和弦也包含 fi $(\uparrow\hat4)$，那么你可能已经识别出了增六和弦。从那里，只需通过查看其余和弦成员来确定具体的子类型（意大利、法国或德国）。

## 大调中的 Ger+6：me 与 ri $(\downarrow\hat3$ 对比 $\uparrow\hat2)$
...
- 增六和弦（.pdf、.docx）。要求学生拼写增六和弦、实现数字低音、用罗马数字写四声部声部进行并分析音乐摘录。音频 1 - 弗雷德里克·肖邦，音频 2 - 斯科特·乔普林（摘录从 0:56 开始）。
---
