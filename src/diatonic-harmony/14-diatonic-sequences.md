...
中间段的自然音序列（Diatonic Sequences） John Peterson

要点总结

- 序列（sequence）是一个被重复和移调的模式。
- 序列分为两大类：上行和下行。下行序列包括：下行五度序列（circle-of-fifths，即五度循环）和下行三度序列（Pachelbel）。上行序列包括：上行 5-6 序列（上行级进）。
- 写作序列时：写出完整的模体（model），写出复制 1 中的完整第一个和弦，遵循音程模式完成其余的复制。
...
我们可以将序列分为两大类：下行的和上行的。在每一类中，某些序列非常常见，以至于我们给它们起了专门的名称。最常见的将在下面介绍。但这里需要强调两点：(1) 这些不是唯一的序列类型。只是这些相当常见。未命名的序列可以简单地标记为"上行序列"或"下行序列"；(2) 不幸的是，音乐理论对下面常见序列有各种不同的命名惯例。我们选择为每个序列给出两个常用名称，但也存在其他命名惯例。
...
写作序列最简单的方法是先建立模体，然后为每个后续复制"复制粘贴"声部进行。注意你的复制应展现出与模体相同的声部进行，但移调到不同的音高层次，否则就不是序列。因此，仔细检查模体中的写作非常重要。你在模体中犯的任何错误都会被重复到所有后续复制中。步骤如下：

- 写出模体。在这里遵循典型的写作程序，优先选择更平滑而非跳跃的高音声部。
- 写出第一个复制的完整第一个和弦。从模体的第一个和弦中复制完全相同的间距。例如，如果模体的和弦 1 中和弦的三音在高音声部，那么每个复制的和弦 1 中和弦的三音也必须在高音声部。
- 确定你为每个声部（高音、中音、次中音、低音）写出的音程模式，并将该模式复制到序列的末尾。你可以通过检查最后一个和弦的间距是否与模体中对应部分匹配来快速检查你是否做对了。如果你在小调中，记住只在属功能和弦解决到主和弦时才升高导音。
